...
Before making any manual changes, run the included interactive bootstrap script to automatically replace all placeholder variables (like `rustarium` and `markurtz`) with your actual project details.
...
> [!NOTE]
> This script will rename the source directory (`src/rustarium/` -> `src/my_cool_project/`) and update all references in the documentation, GitHub Actions workflows, and configuration files.
