...
This directory serves as a generic template for creating new examples in the `rustarium` repository. It demonstrates the standard structure, documentation, and foundational code required for a well-formed example, ensuring it adheres to the latest standards and workflows.
...
- **Configuration (Settings):** How to initialize the `Settings` model using `rustarium.settings.Settings`.
- **Telemetry (Logging):** How to properly configure application logging using `rustarium.logging.LoggingSettings` and `configure_logger` to emit structured logs.
